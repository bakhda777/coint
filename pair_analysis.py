...
def fast_adf(time_series, max_lag=5):
    ...
    try:
        n = len(time_series)
        if n < 20:  # Минимальное кол-во точек для надежного теста
            return 1.0
        
        # Адаптивный выбор лагов (аналог KPSS nlags='auto', но ограничен)
        adaptive_lag = min(max_lag, int(12 * (n / 100) ** 0.25), n // 4)
        max_lag = max(1, adaptive_lag)  # Минимум 1 лаг
        
        # Подготовка данных для ADF
        y_diff = np.diff(time_series)  # Разности ряда
        y_lag = time_series[:-1]      # Лагированные значения
        
        T = n - 1 - max_lag
        if T < 10:  # Недостаточно данных после учета лагов
            return 1.0
        
        # Матрица регрессоров X: константа, лаг уровня, лаги разностей
        X = np.zeros((T, 2 + max_lag))
        Y = y_diff[max_lag:]
        
        for t in range(T):
            X[t, 0] = 1.0                  # Константа
            X[t, 1] = y_lag[t + max_lag]   # Лаг уровня
            for l in range(max_lag):
                X[t, 2 + l] = y_diff[t + max_lag - 1 - l]  # Лаги разностей
        
        # OLS-регрессия через Numba
        XTX = np.dot(X.T, X)
        XTY = np.dot(X.T, Y)
        ...
        try:
            ...
            return 1.0  # Невозможно решить систему
        
        # Остатки и стандартная ошибка
        residuals = Y - np.dot(X, beta)
        dof = T - (2 + max_lag)
        sigma2 = np.sum(residuals ** 2) / dof if dof > 0 else 0.0
        
        try:
            inv_XTX = np.linalg.inv(XTX)
            se_beta = np.sqrt(sigma2 * inv_XTX[1, 1])
        except:
            return 1.0  # Невозможно вычислить стандартную ошибку
        
        # Тестовая статистика
        t_stat = beta[1] / se_beta if se_beta > 1e-10 else 0.0
        
        # Таблица критических значений МакКиннона (для случая с константой)
        crit_values = np.array([-3.43, -2.86, -2.57])  # 1%, 5%, 10%
        p_values = np.array([0.01, 0.05, 0.10])
        
        # Линейная интерполяция p-value
        if t_stat >= crit_values[0]:  # Не стационарен
            return 1.0
        elif t_stat <= crit_values[-1]:  # Сильно стационарен
            return 0.001
        else:
            # Интерполяция между ближайшими критическими значениями
            p_value = 0.5  # Значение по умолчанию
            for i in range(len(crit_values) - 1):
                if crit_values[i] >= t_stat > crit_values[i + 1]:
                    # Интерполяция
                    slope = (p_values[i + 1] - p_values[i]) / (crit_values[i + 1] - crit_values[i])
                    p_value = p_values[i] + slope * (t_stat - crit_values[i])
                    break
            return p_value
            
    except:
        return 1.0  # В случае ошибки считаем, что ряд не стационарен
...
def hybrid_adf(time_series, max_lag=5, autolag='AIC', stationarity_test='adf'):
    """
    Улучшенный гибридный тест на стационарность, использующий оптимизированные реализации.
    
    Ключевые улучшения:
    1. Адаптивный выбор лагов в зависимости от длины ряда
    2. Точная интерполяция p-value на основе таблиц МакКиннона
    3. Полная оптимизация всех вычислений через Numba
    
    Args:
        time_series: numpy array с временным рядом для тестирования
        max_lag: максимальный лаг для включения в модель (будет адаптивно ограничен)
        autolag: стратегия выбора лага ('AIC', 'BIC', 't-stat', или None)
        stationarity_test: тип теста ('adf' - по умолчанию)
        
    Returns:
        p-value: Значение p-value для теста (низкие значения < 0.05 указывают на стационарность)
    """
    try:
        # По умолчанию используем улучшенную Numba-версию
        # для всех величин окон ради производительности
        return fast_adf(time_series, max_lag=max_lag)
            
        # Опция для использования оригинального adfuller для длинных рядов если понадобится (закомментирована):
        # if len(time_series) > 1000 and stationarity_test == 'adfuller':
        #    result = adfuller(time_series, maxlag=max_lag, autolag=autolag)
        #    return result[1]  # Возвращаем только p-value
    except:
        # В случае ошибки используем упрощенную версию
        try:
            return fast_adf(time_series, max_lag=1)
        except:
            # Если и это не сработало, считаем ряд не стационарным
            return 1.0
...
def process_chunk(args: Tuple[List[Tuple[str, str]], pd.DataFrame]) -> List[Dict[str, Any]]:
    ...
    # Убираем ограничение на использование памяти
    ...
    # Обрабатываем каждую пару в чанке
    results = []
    start_time = time.time()
    
    log.info(f"Начало обработки чанка из {len(chunk_pairs)} пар")
    
    for pair_index, (symbol1, symbol2) in enumerate(chunk_pairs):
        pair_start_time = time.time()
        log.info(f"Обработка пары {pair_index+1}/{len(chunk_pairs)}: {symbol1}-{symbol2}")
        
        try:
            # Подготавливаем данные для пары
            merged = prepare_pair_data(df, symbol1, symbol2)
            
            if merged is None or merged.empty:
                log.warning(f"Пропуск пары {symbol1}-{symbol2} - нет данных после объединения")
                results.append({
                    "pair": f"{symbol1}-{symbol2}",
                    "valid": False,
                    "error": "Нет данных после объединения"
                })
                continue
            
            # Получаем цены закрытия
            price1 = merged[f"{symbol1}_close"].values
            price2 = merged[f"{symbol2}_close"].values
            
            # Проверяем валидность данных
            if len(price1) < 100 or len(price2) < 100:
                log.warning(f"Пропуск пары {symbol1}-{symbol2} - недостаточно данных ({len(price1)} точек)")
                results.append({
                    "pair": f"{symbol1}-{symbol2}",
                    "valid": False,
                    "error": f"Недостаточно данных ({len(price1)} точек)"
                })
                continue
                
            try:
                # Рассчитываем коинтеграцию
                _, pvalue, _, _ = coint_numba(price1, price2)
                ...
                result = {
                    ...
                    "timestamp": merged.index[-1].strftime("%Y-%m-%d %H:%M:%S") 
                        if hasattr(merged.index[-1], 'strftime') else str(merged.index[-1])
                }
                results.append(result)
                
                # Логируем результат
                elapsed = time.time() - pair_start_time
                log.info(f"Завершена пара {symbol1}-{symbol2} с p-value={pvalue:.6f} за {elapsed:.2f} сек")
                
            except ValueError as ve:
                log.warning(f"Ошибка данных для {symbol1}-{symbol2}: {str(ve)}")
                results.append({
                    "pair": f"{symbol1}-{symbol2}",
                    "valid": False,
                    "error": str(ve)
                ...
                log.error(f"Ошибка памяти для {symbol1}-{symbol2}: {str(me)}")
                gc.collect()
                results.append({
                    "pair": f"{symbol1}-{symbol2}",
                    "valid": False,
                    "error": f"Недостаточно памяти: {str(me)}"
                })
        except Exception as e:
                log.error(f"Критическая ошибка для {symbol1}-{symbol2}: {str(e)}")
                raise RuntimeError(f"Критическая ошибка при обработке пары {symbol1}-{symbol2}: {str(e)}")
                
        except pd.errors.MergeError as me:
            log.warning(f"Ошибка объединения данных для {symbol1}-{symbol2}: {str(me)}")
            results.append({
                "pair": f"{symbol1}-{symbol2}",
                "valid": False,
                "error": f"Ошибка объединения данных: {str(me)}"
            })
        except Exception as e:
            log.error(f"Критическая ошибка при подготовке данных для {symbol1}-{symbol2}: {str(e)}")
            raise RuntimeError(f"Критическая ошибка при подготовке данных для {symbol1}-{symbol2}: {str(e)}")
        
        # Вызываем сборщик мусора после обработки каждой пары
        gc.collect()
...
# @profile_function('prepare_pair_data')
...
def _mackinnon_p_value_numba(t_stat):
    """
    Точная интерполяция p-value на основе таблицы критических значений МакКиннона для коинтеграции.
    
    Args:
        t_stat: t-статистика для коэффициента при лаге уровня в ADF-регрессии
    
    Returns:
        float: p-value (значение в диапазоне [0.001, 1.0])
    """
    # Таблица критических значений МакКиннона для теста коинтеграции (с константой)
    # Источник: MacKinnon (1991/2010) для теста коинтеграции
    crit_values = np.array([-4.62, -3.92, -3.56, -3.12])  # 1%, 5%, 10%, 20%
    p_values = np.array([0.01, 0.05, 0.10, 0.20])
    
    # Линейная интерполяция p-value
    if t_stat >= crit_values[0]:  # Не коинтегрированы
        return 1.0
    elif t_stat <= crit_values[-1]:  # Сильная коинтеграция
        return 0.001
    else:
        # Интерполяция между ближайшими критическими значениями
        for i in range(len(crit_values) - 1):
        ...
        # Если по какой-то причине не нашли интервал
        # Используем значение по умолчанию
        if t_stat > -2.0:
            return 0.9  # Очень далеко от коинтеграции
        else:
            return 0.3  # Близко к границе коинтеграции
...
def coint_numba(x, y, maxlag=1):
    ...
    # Сначала пробуем параллельную версию
    try:
        t_stat, pvalue, crit, resid = _coint_numba_njit(x, y, maxlag)
        success = True
    ...
    # Если параллельная версия не сработала, пробуем однопоточную
    if not success:
        log.warning(f"Параллельная версия coint_numba не удалась: {error_message}. Используем однопоточную версию.")
        
        try:
            t_stat, pvalue, crit, resid = _coint_numba_njit_single(x, y, maxlag)
        except Exception as e2:
            # Если и однопоточная версия не сработала, пытаемся рассчитать приближенно
            log.error(f"Обе версии coint_numba не удались. Вторая ошибка: {str(e2)}")
            raise ValueError(f"Невозможно вычислить коинтеграцию: {error_message}, затем {str(e2)}")

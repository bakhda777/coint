...
# pip install pandas numpy pyarrow numba scipy joblib bottleneck
...
def backtest_one_pair(pair, price_data, params, dates_index=None):
    """
    ...
    position_open = False
    position_side = 0  # 0: нет позиции, 1: лонг x / шорт y, -1: шорт x / лонг y
    entry_index = 0
    entry_time = None
    entry_prices = (0, 0)
    ...
    rolling_window = params.rolling_window  # 5760 точек (60 дней × 24 часа × 4 бара в час)
    
    for i in range(rolling_window, len(log_px1_np)):
        # Выделяем окно для расчета бета и спреда
        # Берем именно последние params.rolling_window точек
        window_start = i - params.rolling_window
        window_end = i
        
        # Используем numpy-массивы вместо pandas-серий для текущего окна
        x_window = log_px1_np[window_start:window_end]
        y_window = log_px2_np[window_start:window_end]
        
        # Проверка на достаточное количество данных без NaN
        nan_count_x = np.isnan(x_window).sum()
        nan_count_y = np.isnan(y_window).sum()
        
        if nan_count_x > params.rolling_window * 0.1 or nan_count_y > params.rolling_window * 0.1:
            logging.debug(f"Пропуск бара из-за большого количества NaN в окне для пары {symbol1}-{symbol2}")
            continue
            
        # Очистка от NaN для расчета бета
        mask = ~(np.isnan(x_window) | np.isnan(y_window))
        x_window_clean = x_window[mask]
        y_window_clean = y_window[mask]
        
        # Шаг 10: Проверка на наличие достаточного числа ненулевых наблюдений
        # Если после удаления NaN в окне оказалось меньше 0.9 × params.rolling_window точек, пропустить этот timestamp
        min_count = int(params.rolling_window * 0.9)
        if len(x_window_clean) < min_count or len(y_window_clean) < min_count:
            logging.debug(f"Пропуск бара из-за недостаточного количества данных после очистки от NaN для пары {symbol1}-{symbol2} (необходимо минимум {min_count} точек)")
            continue

        # Шаг 11: Перед вычислением z-score вызвать fast_coint_numba_final из test_precise.py
        # и получить tau
        try:
            from test_precise import fast_coint_numba_final
            
            # ВНИМАНИЕ! ВАЖНЫЙ КОММЕНТАРИЙ ДЛЯ РАЗРАБОТЧИКОВ:
            # Шаг 11-12: Явно указываем, что fast_coint_numba_final проверяет коинтеграцию за последние 5760 точек
            # В данном бэктесте используется СТРОГО скользящее окно из 5760 точек (60 дней × 24 часа × 4 бара в час).
            # На каждом шаге вызывается функция fast_coint_numba_final для проверки коинтеграции на этом окне.
            # БЕЗ ВЫПОЛНЕНИЯ ЭТОГО ТЕСТА ПОЗИЦИЯ НЕ ОТКРОЕТСЯ!
            # Если p-value > params.p_value_threshold (0.05), то позиция не открывается.
            #
            # Для новых разработчиков: бэктест выполняется строго по окну из 5760 точек.
            # На каждом шаге выполняется тест на коинтеграцию с использованием fast_coint_numba_final.
            # Если пара не проходит тест (p-value > 0.05), то позиция не открывается.
            
            # Вызов функции fast_coint_numba_final для проверки коинтеграции
            # за последние 5760 точек (60 дней × 24 часа × 4 бара)
            tau, _, _ = fast_coint_numba_final(x_window_clean, y_window_clean)
        except Exception as e:
            logging.error(f"Ошибка при вызове fast_coint_numba_final: {e}")
            continue

        # Расчет массива бета-коэффициентов для всех окон
        # Для текущего окна
        beta = calc_beta(x_window_clean, y_window_clean, use_kalman=params.use_kalman, delta=params.delta)
        ...
        x_current = log_px1_np[i]
        y_current = log_px2_np[i]
        price1 = prices1_np[i]
        price2 = prices2_np[i]
        
        # Создаем массив спредов для всего лога цен
        # Создаем единый массив spread_array = log_x_array - β_array * log_y_array
        spread_array = log_px1_np - beta * log_px2_np
        
        # Задаем для тех i, где beta равен nan, значение spread_array[i] = nan
        if np.isnan(beta):
            spread_array[:] = np.nan
        ...
        # Используем bottleneck для расчета скользящего среднего и стандартного отклонения
        try:
            # Вызываем mu_array = bn.move_mean(spread_array, window=W, min_count=min_count)
            mu_array = bn.move_mean(spread_array, window=W, min_count=min_count)
            # Вызываем sigma_array = bn.move_std(spread_array, window=W, min_count=min_count, ddof=0)
            sigma_array = bn.move_std(spread_array, window=W, min_count=min_count, ddof=0)
        except Exception as e:
            logging.warning(f"Ошибка при использовании bottleneck: {e}. Используем собственную реализацию.")
            # Если bottleneck недоступен, используем нашу JIT-функцию
            mu_array, sigma_array = welford_moving_stats(spread_array, W, min_count)
        
        # Проверяем, что mu_array и sigma_array имеют одинаковую длину и корректно обрабатывают позиции с NaN
        if len(mu_array) != len(sigma_array) or len(mu_array) != len(spread_array):
            logging.error(f"Ошибка: массивы mu_array и sigma_array имеют разную длину для пары {symbol1}-{symbol2}")
            continue
        
        # Получаем текущие значения mu и sigma
        mu = mu_array[i]
        sigma = sigma_array[i]
        ...
        # Расчет текущего спреда и z-score для текущего i
        spread = spread_array[i]  # Уже вычислено ранее как log_px1_np[i] - beta * log_px2_np[i]
        z = (spread - mu) / sigma
    ...
    if not trades:
        logging.info(f"Нет сделок для пары {symbol1}-{symbol2}")
        return pd.DataFrame()
    ...
    logging.info(f"Всего сделок для пары {symbol1}-{symbol2}: {len(trades_df)}")
...
def backtest_all_pairs(pairs, price_data, params, n_jobs=1, dates_index=None):
    ...
    # Используем joblib для параллельного выполнения
    results = Parallel(n_jobs=n_jobs)(delayed(backtest_one_pair)(pair, price_data, params, dates_index) for pair in pairs)
...
if __name__ == "__main__":
    ...
    # Запуск бэктестинга на небольшом количестве пар для демонстрации
    logging.info("Запуск бэктестинга на первых 10 парах...")
    all_trades = backtest_all_pairs(pairs[:10], pair_arrays, params, n_jobs=1, dates_index=dates_index)  # Используем последовательное выполнение

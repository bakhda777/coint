...
def _adf_autolag_numba_final(res, du_precomputed, k_max):
    ...
    # Делаем непрерывным один раз до цикла
    ...
    for k in range(k_max + 1):
        ...
        # Важно: обеспечиваем непрерывность массива для оптимальной производительности
        X = np.ascontiguousarray(X_max[:n_eff, :m])
        X[:, 0] = res[k:-1]
        
        # Формируем матрицу регрессоров точно как в statsmodels
        if k > 0:
            # Оптимизированное заполнение матрицы регрессоров без копирования
            for j in prange(k):

...
# Функция compute_aic_optimized удалена, так как мы фиксируем лаг k=2 и не используем AIC
...
def _adf_autolag_numba_final(res, du_precomputed, k_max):
    ...
    # Для k=2 сразу берем зависимую переменную - разности начиная с индекса 2
    y = du_precomputed[2:]
    
    # Заполняем матрицу регрессоров X_local за три векторизированных присваивания
    # Столбец 0: уровни ряда res[2:-1]
    X_local[:, 0] = res[2:-1]
    # Столбец 1: первый лаг разностей du_precomputed[1:n-2]
    X_local[:, 1] = du_precomputed[1:n-2]
    # Столбец 2: второй лаг разностей du_precomputed[0:n-3]
    X_local[:, 2] = du_precomputed[0:n-3]
            
    # Решаем нормальные уравнения через разложение Холецкого
    # X_local и y уже являются C-континьюи, нет необходимости в дополнительных копиях
    xtx = X_local.T @ X_local
    xty = X_local.T @ y
